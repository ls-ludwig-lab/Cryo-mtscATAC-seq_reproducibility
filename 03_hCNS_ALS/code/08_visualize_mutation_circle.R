...
samples <- list(
  Region_5 = readRDS(".../output/Region_5/output/variants_ncof1.rds"),
  Region_10 = readRDS(".../output/Region_10/output/variants_ncof1.rds"),
  Region_28 = readRDS(".../output/Region_28/output/variants_ncof1.rds"),
  Region_29 = readRDS(".../output/Region_29/output/variants_ncof1.rds")
)
...
for (sample_name in names(samples)) {
  ...
  p <- ggplot(df, aes(x = position, y = mean, colour = sample_name)) +
    geom_point(size = 0.8) +
    scale_color_manual(values = c(region.colors)) +
