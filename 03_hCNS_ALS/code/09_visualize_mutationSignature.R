...
ref_all <- fread("/.../final/chrM_refAllele.txt")
...
called_variants1 <- rowData(readRDS(".../variants_ncof1.rds"))$variant
called_variants2 <- rowData(readRDS("../variants_ncof1.rds"))$variant
called_variants3 <- rowData(readRDS("../variants_ncof1.rds"))$variant
called_variants4 <-rowData(readRDS("../variants_ncof1.rds"))$variant
...
results <- lapply(seq_along(called_variants_list), function(i) {
  ...
  # Save the plot
  output_file <- paste0(".../output/mito_signature_", i, ".pdf")
