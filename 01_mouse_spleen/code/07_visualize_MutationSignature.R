...
ref_all <- fread(".../FF_10min/data/mgatk_CR-ATAC_mSpleen_FF_10min_mtMask/final/chrM_refAllele.txt")
...
called_variants1 <- rowData(readRDS(".../Cells/output/variants_ncof0.rds"))$variant
called_variants2 <- rowData(readRDS(".../FF_10min/output/variants_ncof0.rds"))$variant
called_variants3 <- rowData(readRDS(".../FF_30min/output/variants_ncof0.rds"))$variant
...
called_variants_list <- list(called_variants1,called_variants2, called_variants3)
...
results <- lapply(seq_along(called_variants_list), function(i) {
  ...
  # Save the plot
  output_file <- paste0(".../integration/output/mito_signature_", i, ".pdf")
