...
donor0 <- subset(df, vireo_donor == "donor0")
donor1 <- subset(df, vireo_donor == "donor1")


#Subset to HQ cells that exist so far
common_cols <- intersect(colnames(SE), colnames(donor0))
SE0 <- SE[, common_cols]

common_cols <- intersect(colnames(SE), colnames(donor1))
SE1 <- SE[, common_cols]

variantsD0 <- call_mutations_mgatk(SE0)
variantsD1 <- call_mutations_mgatk(SE1)

saveRDS(variantsD0, "variants_Donor0.rds")
saveRDS(variantsD1, "variants_Donor1.rds")
