...
primary <- readRDS(file.path(input_dir, "variants_donor0.rds"))
...
for (threshold in thresholds) {
  ...
  variants_csv_path <- paste0(file.path(output_dir), threshold, "_Donor0variants.csv")
  misc_df_csv_path <- paste0(file.path(output_dir), threshold, "_Donor0.csv")
  rds_path <- paste0(file.path(output_dir), threshold, "_Donor0.rds")
