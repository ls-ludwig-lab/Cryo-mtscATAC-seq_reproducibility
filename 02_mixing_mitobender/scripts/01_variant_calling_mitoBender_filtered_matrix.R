...
#The original mgatk output files are not available within this repository due to human genotyping data.

##Script for variant calling using the mitoBender filtered matrix

SE <- readRDS(".../mitoBender/mitoBender_filtered.rds")

#mitoBender file do not have the reference genome - rowRanges from original 
df1 <- readRDS("/mgatk_output.rds")

rowRanges(SE) <- rowRanges(df1)
...
primary <- call_mutations_mgatk(SE)
df1
head(primary)

# the mitoBender file contains 2000 cols - subset to the original mgatk file barcodes
matching_cells <- colnames(df1)[colnames(df1) %in% colnames(primary)]
matching_cells
primary <- primary[, matching_cells]
primary
